var loginControl = function (formId) {
	...
	$.ajax({
		url: 'cfc/ajax.cfc?method=checkLogin', data: { data: data }, async: false, success: function (res) {
			data = $.parseJSON(res);
			console.log(data);
			if (data['DATA'].length) {
				window.location.href = 'index.cfm?pageinfo=welcome';
			}
			else {
				alert('Hatalı giriş');
			}
...
var logOut = function () { $.ajax({ url: 'cfc/ajax.cfc?method=logOut', async: false, success: function (res) { window.location.href = 'index.cfm?pageinfo=welcome'; } }); }
